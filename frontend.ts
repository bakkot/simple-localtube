    ...
    <div class="topright-info">
      <span class="settings">
        ...
        <div class="settings-dropdown">
          ...
          ${subscriptionsDb && permissions.canSubscribe ? '<a href="/subscriptions">Subscriptions</a><a href="/add-video">Add Video</a>' : ''}
...
const addVideoTemplate = parseTemplate(fs.readFileSync(path.join(templates, 'add-video.html'), 'utf8'));
export function renderAddVideoPage(username: string, permissions: Permissions, videoQueue: import('./subscriptions-db.ts').QueuedVideo[]): string {
  return applyTemplate(addVideoTemplate, {
    commonCSS,
    formPageCSS,
    topRightBlock: renderTopRightBlock(username, permissions),
    canSubscribe: permissions.canSubscribe,
