...
export async function getLatestVideoUrls(channelId: string): Promise<string[]> {
  ...
  while (!foundKnownVideo) {
    ...
    try {
      ...
      const currentBatchNewUrls: string[] = [];
      for (const url of batchUrls) {
        const videoId = extractVideoId(url);

        if (isVideoInDb(videoId)) {
          console.log(`Found known video ${videoId} in DB. Stopping search.`);
          // Add URLs *before* this known one from the *current* batch
          newVideoUrls.push(...currentBatchNewUrls);
          foundKnownVideo = true;
          break; // Stop processing this batch
        } else {
          // Add to list of potential new videos for this batch
          currentBatchNewUrls.push(url);
        }
      }

      // If we finished the batch without finding a known video, add all from this batch
      if (!foundKnownVideo) {
        newVideoUrls.push(...currentBatchNewUrls);
        startIndex = endIndex + 1; // Prepare for the next batch
        ...
        if (batchUrls.length === YT_DLP_BATCH_SIZE) {
          ...
          console.log('Reached end of playlist in this batch, not pausing.');
          break; // Optimization: yt-dlp returned fewer than requested, likely end of playlist
        }
      }
    } catch (error: any) {
