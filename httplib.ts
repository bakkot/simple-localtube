...
export type Handler<Ctx extends object = object> = (req: HttpRequest, ctx: Ctx, rawRes: http.ServerResponse) => void | Promise<void>;
export type Middleware<Extra extends object> = (req: HttpRequest, rawRes: http.ServerResponse, next: (extra: Extra) => void | Promise<void>) => void | Promise<void>;

type RouteSegment = { kind: 'literal'; value: string } | { kind: 'param'; name: string };

type InternalHandler = (req: HttpRequest, rawRes: http.ServerResponse) => void;
type InternalHandlerWithCtx<Ctx extends object = object> = (req: HttpRequest, ctx: Ctx, rawRes: http.ServerResponse) => void | Promise<void>;

interface CompiledRoute<Ctx extends object = object> {
  method: 'GET' | 'POST';
  segments: RouteSegment[];
  handler(req: HttpRequest, ctx: Ctx, rawRes: http.ServerResponse): void | Promise<void>;
}
    ...
    _wrapHandler: (inner) => prevWrap(async (req, ctx, rawRes) => {
      try {
        await mw(req, rawRes, async (extra) => {
          await inner(req, { ...ctx, ...extra }, rawRes);
        });
      } catch (err) {
...
export function addGetRoute<Ctx extends object, Path extends string>(app: App<Ctx>, pattern: Path, handler: (req: HttpRequest<ExtractRouteParams<Path>>, ctx: Ctx, rawRes: http.ServerResponse) => void | Promise<void>): void {
  app.routes.push({
    method: 'GET',
...
export function addPostRoute<Ctx extends object, Path extends string>(app: App<Ctx>, pattern: Path, handler: (req: HttpRequest<ExtractRouteParams<Path>>, ctx: Ctx, rawRes: http.ServerResponse) => void | Promise<void>): void {
  app.routes.push({
    method: 'POST',
    segments: compilePattern(pattern),
    handler,
...
export function send(rawRes: http.ServerResponse, body: string | Buffer): void {
...
export function sendJson(rawRes: http.ServerResponse, obj: unknown): void {
...
export function redirect(rawRes: http.ServerResponse, url: string): void {
  rawRes.statusCode = 302;
  rawRes.setHeader('Location', url);
  rawRes.end();
}

export async function sendFile(req: HttpRequest, rawRes: http.ServerResponse, absolutePath: string): Promise<void> {
  let stat: fs.Stats;
  try {
...
export function listen<Ctx extends object>(app: App<Ctx>, port: number, cb: (err?: Error) => void): http.Server {
  const handler = app._wrapHandler(async (req, ctx, rawRes) => {
    for (const route of app.routes) {
      const params = matchRoute(route, req.method, req.path);
      if (params) {
        req.params = params;
        try {
          await route.handler(req, ctx, rawRes);
        } catch (err) {
          handleError(err, rawRes);
        }
        return;
